...
# Bestimmung p-Wert
pwert <- prop( ~ abs(dayTypeweekend) >= abs(betadach[2]), 
               data = Nullvtlg)
pwert
```
...
Der p-Wert ist die Wahrscheinlichkeit dafür, dass ein Wert auftritt, der mindestens so weit vom Wert der Nullhypothese (hier $\beta_\text{dayTypeweekend} = 0$) entfernt ist, wie der in der Stichprobe beobachtete Punktschätzer (hier $\hat\beta_\text{dayTypeweekend} \approx `r betadach[2] |> round(2)`$).
